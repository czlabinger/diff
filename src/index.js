...
module.exports = class DiffSyncAlghorithm {
  ...
  initObject(container, mainText) {
    ...
    if (mainText !== null && mainText !== undefined) {
      ...
      // Create backup if useBackup is true
      if (useBackup) {
        container.backup = {
          [thisVersion]: 0,
          [senderVersion]: 0,
      ...
      // If text is not defined create empty shadow and backup
      container.shadow = {};
      container.backup = {};
    }
  }
  ...
  onReceive({ payload, container, onUpdateMain, afterUpdate, onUpdateShadow }) {
    ...
    // Get edit that are newer than the shadow version
    const filteredEdits = payload.edits.filter(
      ...
      for (let patch of patches) {
        if (patch.length > 0) {
          ...
          if (onUpdateShadow) {
            ...
            shadow.value = jsonpatch.applyPatch(
              shadow.value,
              patch,
            ).newDocument;
          }
  ...
  clearOldEdits(shadow, version) {
    shadow.edits = shadow.edits.filter(
      (edit) => edit[this.thisVersion] > version,
    );
  }
